...
$softwareName = 'Microsoft Report Viewer 2015 Runtime*'
$installerType = 'MSI'
$UninstallGuid =  '{3ECE8FC7-7020-4756-A71C-C345D4725B77}'
...
if ($key.Count -eq 1) {
  if ($validExitCodes -NotContains (Start-Process -FilePath msiexec -ArgumentList "/x $UninstallGuid $silentArgs" -Wait -Passthru).ExitCode)
        {
            Throw " Microsoft SCOM is probably registered, try to force unregistering/disable compliance"
        }
